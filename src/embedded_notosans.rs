use noto_sans_mono_bitmap::{FontWeight, get_raster, RasterHeight};
use pixels_graphics_lib::buffer_graphics_lib::Graphics;
use pixels_graphics_lib::MouseData;
use pixels_graphics_lib::prelude::{BLACK, Color, KeyCode, Scene, SceneUpdateResult, Timing};
        ...
        for (idx,chr) in "Hello world".chars().enumerate() {
            ...
            for (y, row) in raster.raster().iter().enumerate() {
                for (x, pixel) in row.iter().enumerate() {
                    ...
                    graphics.set_pixel((start.0 + x ) as isize, (start.1 + y) as isize, Color::gray(*pixel));
                }
            }
        }
    }
